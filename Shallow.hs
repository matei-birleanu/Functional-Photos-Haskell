...
rectangle width height = \point ->  let (x, y) = point 
                                        in x >= -width / 2 && x <= width / 2 && y <= height / 2 && y >= -height / 2
...
translation tx ty = \point -> let (x, y) = point
                                in (x - tx, y - ty)
...
scaling factor = \point -> let (x, y) = point
...
bfs start expand = bfsHelper [(start, 0)] S.empty
  where
    bfsHelper [] _ = []
    bfsHelper ((s, d):xs) visited =
      if s `S.member` visited
        then bfsHelper xs visited
        else (s, d) : bfsHelper (xs ++ newStates) (S.insert s visited)
      where
        newStates = [(n, d + 1) | n <- expand s, n `S.notMember` visited]
...
{-
    *** TODO BONUS ***
    ...
    .........................................
-}
expandPoint :: Region -> Point -> [Point]
expandPoint region = filter (not . region) . neighbors
  where
    neighbors (x, y) = [(x, y+1), (x+1, y), (x, y-1), (x-1, y)]

regionAvoidingBfs :: Point -> Region -> [(Point, Int)]
regionAvoidingBfs start region = bfs start (expandPoint region)
