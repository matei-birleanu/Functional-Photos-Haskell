...
instance Show RegionAST where
    show = init . showRegion 0 

indent :: Int -> String
indent level = replicate (level * 2) ' '

showRegion :: Int -> RegionAST -> String
showRegion level (R r) = case r of
    FromPoints points ->
        indent level ++ "FromPoints " ++ show points ++ "\n"
    Rectangle w h ->
        indent level ++ "Rectangle " ++ show w ++ " " ++ show h ++ "\n"
    Circle radius ->
        indent level ++ "Circle " ++ show radius ++ "\n"
    Complement sub ->
        indent level ++ "~\n" ++ showRegion (level + 1) sub
    Union left right ->
        indent level ++ "+\n" ++
        concatMap (showRegion (level + 1)) [left, right]
    Intersection left right ->
        indent level ++ "*\n" ++
        concatMap (showRegion (level + 1)) [left, right]
    Transform transformation sub ->
...
instance Num RegionAST where
    -- `fromInteger` maps an integer n to the region containing only the point (n, n).
    fromInteger n = fromPoints [(fromInteger n, fromInteger n)]

    -- Negation corresponds to taking the complement of a region.
    negate = complement

    -- Addition of two regions corresponds to their union.
    (+) = union

    -- Multiplication of two regions corresponds to their intersection.
    (*) = intersection
